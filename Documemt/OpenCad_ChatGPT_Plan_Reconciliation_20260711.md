...
3. **WP1-7 Vertical Slice A（真正的 Phase 1 Gate）**：`vertical-slice-a.ps1` 已寫但**從未在 freecad 引擎實跑**。這是 Phase 1 完成的定義，也是「維持路線 B」假設的最大驗證點。跑通後才把預設引擎切 freecad（WP1-0R 報告 §H 明列此順序）。
