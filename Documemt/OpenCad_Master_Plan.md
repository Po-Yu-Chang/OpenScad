...
> 最後更新：2026-07-12（依 codebase 全面盤點校正，證據見 `OpenCad_Codebase_Review_20260712.md`；前版依 Gap Review 改版）
> **Phase 0 Gate 已通過**：路線 B 續行（Avalonia＋FreeCAD 權威核心），見 `Phase0_Engine_Decision.md`。
> 進度速覽：Phase 0 全部✅、包 A/B/C/D✅、WP1-0R 引擎接線✅（但 FreeCAD adapter 僅 9/22 特徵）、**WP1-7-UI ✅（2026-07-12：雙引擎 vertical-slice 11/11＋smoke-test PASS，見 §15 序 10）**。⚠ WP1-2 有紅線違規：rebuild 不求解約束（§1.6）。下一包＝WP1-2R（真求解器）→ WP1-0R2（§15 序 10a–10b）。
...
### 1.6 2026-07-12 Codebase 盤點校正（哪些 ✅ 要打折——完整證據見 `OpenCad_Codebase_Review_20260712.md`）

**紅線違規（最優先）**：
- **WP1-2 草圖約束**：求解器是自寫 heuristic 投影（`sketch_solver.py`，非 FreeCAD Sketcher、非聯立求解）；只在互動 solve 端點生效，**rebuild 時兩個 adapter 都不讀 `feature.constraints`＝死 metadata**——正是 §0.2 鐵則 3 永久禁止的形態。angle/symmetric/tangent 三約束無幾何實作（測試只驗型別存在＝假綠）；DOF＝固定成本表、衝突集＝標最後 N 個，皆非真實分析。真 FreeCAD Sketcher 只在 `cad-worker-freecad/` spike，其 18 個 solver 測試任何自動化路徑都不執行。→ **WP1-2R**。

**宣稱與實際的落差**：
- **FreeCAD adapter 僅 9/22 特徵**（缺 sweep/loft/mirror/shell/boolean×3＋WP1-6 六型，`freecad_adapter.py:409` 缺型丟 ValueError）；`FREECAD_ADAPTER_LIMITATIONS.md` 能力矩陣宣稱 feature-complete **不實**。→ **WP1-0R2**。
- **WP1-7**：引擎層 pytest 12/12 在 freecad 通過（2026-07-12）；**UI 層 `vertical-slice-a.ps1` 從未實跑**，且腳本有三缺陷（無 token 時 exit 0 no-op、缺 step 2、step 9 存/重開是同專案連 GET 兩次恆真）＋一個致命 bug（`$pid` 唯讀自動變數賦值，一執行就拋錯——證明從未跑過）。→ **WP1-7-UI ✅ 已於 2026-07-12 完成**（腳本重寫＋實跑雙引擎 11/11，見 §15 序 10）。
- **WP1-3 datum 降級為部分**：`_resolve_face` 硬編方位、`_resolve_vertex` 回原點；FreeCAD 下 datum 草圖面退回 XY（佔位）；UI 建立對話框硬編 `face:f1.top`+10mm。
- **build123d 品質瑕疵**：`_build_draft` 是 no-op（TODO WP2）、`_build_variable_fillet` 退化單半徑。
- **契約同步破口**：C# CommandValidator 對 WP1-6 六型無驗證（與 Python 不對稱）；C# LLM prompt feature enum 混入 datum 三型；`feature.schema.json` 的 reference_geometry 定義孤立未接頂層；reorder_feature 後端齊全但 UI 未接線。→ **WP-S1**。
- **topology.py 語意解析器**「僅測試使用，尚未接進 adapter」；edge disambiguation by centroid 未實作、以 skip 迴避。
- **Phase 2-4 功能（equations/configurations 套用/multi-body 行為/surface/assembly/mates/BOM/drawing/鈑金/焊件）codebase 中不存在或只有 schema round-trip——符合計畫（未發包），非掉球**。
- 文件矛盾：`FREECAD_TEST_RESULTS.md`（0 bug、2.93s）vs `WP1-0R_Report.md`（修 1 個 ShapeWrapper bug、133.95s）；WP1-0R 報告的 smoke-test PASS 宣稱早於其後兩個修復 commit（自標「部分修復」）。
- 工作區：5 個 Desktop UX 修補（例外浮現/disabled 樣式/未連線橫幅）**未 commit、無測試**；本地領先 origin 13 commit 未 push；根目錄 `FreeCAD_Packaging_Notes.md` 與 `Documemt/` 版重複。
- 測試計數說明：916 Python／173 .NET 是含 parametrize/Theory 展開的執行數；獨立測試函式約 410／82。`tests/geometry`、`tests/unit`、`tests/golden-models` 三目錄為空。
...
21. **（新）constraints 在 rebuild pipeline 是死資料**（WP1-2R 完成前）：adapter `_build_sketch` 只讀 `sketch_entities` 座標，「約束已滿足」全靠互動 solve 寫回的座標。任何繞過 solve 直接改 entities 的程式路徑（LLM 生成、migration、手改 JSON）都會產生違反約束的幾何且無人發現。修 WP1-2R 前，新程式碼不得假設 constraints 會被強制。
22. **（新）假綠三態——看到「全綠」先驗證跑了什麼**：(a) angle/symmetric/tangent 測試只驗型別存在不驗幾何；(b) FreeCAD 測試在系統 Python 3.12 下全 skip（只有 cp311 `bin\python.exe` 實跑，`run_freecad_tests.bat` 也只跑 adapter 測試）；(c) ~~`vertical-slice-a.ps1` 無 token 時 exit 0 no-op 綠燈~~（2026-07-12 已修：token 自給、缺 token 一律 FAIL）。驗收報告必須註明引擎、python 版本、實際收集的測試數。
23. **（新）UIA 驅動 Avalonia 選單只能走鍵盤路徑**（2026-07-12 實測）：Avalonia MenuItem 在 UIA 只暴露 ScrollItemPattern——ExpandCollapse/Invoke pattern 都會丟 Unsupported；實體滑鼠座標點擊受 DPI 虛擬化影響不可靠。穩定做法＝`SetFocus()`→SendKeys `{ENTER}`（展開）→`{DOWN}{ENTER}`（選項），並以「子選單項出現在 UIA 樹」確認真的展開。另：名稱比對必須限定 ControlType（MenuItem/Button）——純字串比對會誤中聊天訊息 TextBlock 與開始頁專案卡片（`-like` 不分大小寫，"NEMA17" 會中 "nema17-mount"）。
24. **（新）tests/prompts 有 `__init__.py`（是 package）**：pytest 只把 `tests/` 加進 sys.path，`from opencad_llm_validator import ...` 會 ModuleNotFoundError——靠 `tests/prompts/conftest.py` 把本目錄補進 sys.path（2026-07-12 已加，勿刪）。
...
| 序 | 包 | 前置 | 狀態（2026-07-12 codebase 盤點校正，證據見 `OpenCad_Codebase_Review_20260712.md`） |
|---|---|---|---|
| 0 | 包 A（Enter bug） | 無 | ✅（⚠中文 IME 驗收 2 待人工實測） |
| 1 | WP0-3（display map＋picking）／WP0-1（FreeCAD spike） | 無 | ✅ |
| 2 | WP0-4（語意參照＋sweep）／WP0-2（solver kill test） | WP0-3／WP0-1 | ✅（⚠ `topology.py` 解析器僅測試使用未接 adapter；edge disambiguation by centroid 未實作、以 skip 迴避） |
| 3 | 包 B（上下文記憶）／包 D（草圖平面） | 無 | ✅ |
| 4 | **WP0-5 決策 Gate** | WP0-1..4 | ✅ 路線 B 續行 |
| 5 | 包 C（triad＋基準面）／WP1-0R（FreeCAD 正式化收尾） | Gate 過 | ✅ 引擎接線＋36 adapter 測試（cp311）＋replay 12/12；⚠ **FreeCAD adapter 僅 9/22 特徵**（→序 10b WP1-0R2）、預設引擎仍 build123d、`FREECAD_ADAPTER_LIMITATIONS.md` 矩陣不實 |
| 6 | WP1-1（Document Model v2） | WP1-0 | ✅（rollback bar UI 殘留；reorder_feature 後端齊全但 **UI 未接線**→WP-S1） |
| 7 | WP1-2（真 Sketcher）／WP1-5（檔案強化） | WP1-1 | WP1-5 ✅／WP1-2 **降級為部分**：互動 solve（11/14 約束）＋UI 完整，但 **rebuild 不求解＝鐵則 3 紅線違規**、angle/symmetric/tangent 空殼、DOF/衝突為 heuristic（→序 10a WP1-2R） |
| 8 | WP1-3（基準幾何）／WP-H1（LLM 收斂） | WP1-2 | WP1-3 **降級為部分**（datum 解析硬編/佔位、FreeCAD 退回 XY、UI 對話框 demo 級→序 10c）／WP-H1 程式碼✅、真 gateway 實測待補（tests/prompts 為模擬 plan 非端到端） |
| 9 | WP1-4（Property Manager/量測）／WP1-6（特徵補全）／WP-H2（安全） | WP1-3 | 程式碼✅（⚠ WP1-6：build123d draft 為 no-op、variable_fillet 退化單半徑、golden test 僅 build123d；C# validator 未覆蓋六型→WP-S1） |
| **10** | **WP1-7-UI：Vertical Slice A UI 層＝Phase 1 Gate** | WP1-1..6＋WP1-0R | ✅ **2026-07-12 完成**：(1) 5 個 Desktop UX 修補已 commit＋10 個 RelayCommand 錯誤處理測試（.NET 138 全綠）(2) `vertical-slice-a.ps1` 重寫（$pid bug、token 自給、補 step 2、step 9 真重啟重開、假斷言全強化）→ **實跑 build123d 11/11＋freecad 11/11 PASS** (3) smoke-test 修復（鍵盤選單導覽＝地雷 #23、PS5.1 Join-Path）→ **build123d PASS**；⚠ freecad 引擎 smoke-test 被 NEMA17 範例的 shell 特徵擋下（FreeCAD adapter 未實作 shell）→ 歸 WP1-0R2。預設引擎維持 build123d |
| 10a | **WP1-2R 真求解器（新增）** | WP1-7-UI | rebuild 對含 constraints 的 sketch 重新求解（FreeCAD Sketcher 為權威，或至少 rebuild 驗證約束殘差、不符即 fail）；補 angle/symmetric/tangent 幾何實作；真 DOF（rank）與真衝突集；把 `cad-worker-freecad/tests/test_sketch_solver.py` 18 個測試接進 cp311 執行路徑；解除地雷 #21 |
| 10b | **WP1-0R2 FreeCAD 特徵 parity（新增）** | WP1-2R | 補 13 個缺失特徵（優先 sweep/loft/mirror/shell/boolean×3，WP1-6 六型次之）；golden tests 參數化雙引擎；改寫 `FREECAD_ADAPTER_LIMITATIONS.md`；完成後才決定切預設引擎 |
| 10c | **WP-S1 契約同步＋WP1-3 收尾（新增，小包）** | 可與 10a/10b 並行 | C# CommandValidator 補 WP1-6 驗證；LLM prompt 移除 datum 三型；feature.schema reference_geometry 接線；reorder UI；datum `_resolve_face`/`_resolve_vertex` 接真 BREP、UI 對話框真選面；刪根目錄重複 `FreeCAD_Packaging_Notes.md`；清理三個空測試目錄 |
| 11 | Phase 2 各包（終稿後發）→ Phase 3 → Phase 4 | 10–10c 全過 | 未動工（configurations/global_variables 僅 schema round-trip，assembly/BOM/drawing/鈑金/焊件 codebase 0 實作——符合計畫） |
