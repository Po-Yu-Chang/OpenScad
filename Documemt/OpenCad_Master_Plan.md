...
> 最後更新：2026-07-11（commit `6fba1c4` 之後；依 `OpenCad_SolidWorks_Gap_Review_20260711.md` 全面改版）
...
| 3 | 包 B（上下文記憶）／包 D（草圖平面）——穿插於等待期 | 無 | ✅ |
| 4 | **WP0-5 決策 Gate** | WP0-1..4 | ✅ 通過 |
| 5 | 包 C（triad＋基準面）／WP1-0（FreeCAD 正式化＋WP-H4） | Gate 過 | ✅ |
| 6 | WP1-1（Document Model v2） | WP1-0 | ✅ |
| 7 | WP1-2（真 Sketcher）／WP1-5（檔案強化） | WP1-1 | WP1-5 ✅／WP1-2 ✅ |
| 8 | WP1-3（基準幾何）／WP-H1（LLM 收斂） | WP1-2 | WP1-3 ✅／WP-H1 ✅ |
| 9 | ~~WP1-4（Property Manager/量測）~~ ✅ ／ ~~WP1-6（特徵補全）~~ ✅ ／ ~~WP-H2（安全）~~ ✅ | WP1-3 | |
| 10 | ~~**WP1-7 Vertical Slice A＝Phase 1 Gate**~~ ✅ | WP1-1..6 | |
